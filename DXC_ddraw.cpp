...
DXC_ddraw::DXC_ddraw()
{
	...
	m_bFullMode		= false;
#else
	// Force borderless fullscreen (windowed) to avoid exclusive fullscreen latency.
	m_bFullMode		= false;
#endif
	res_x = 0;
	res_y = 0;
...
bool DXC_ddraw::bInit(HWND hWnd)
{
 ...
 int            iS, iD;
	...
	// Use NULL to select the primary display device
	ddVal = DirectDrawCreateEx(NULL, (VOID**)&m_lpDD4, IID_IDirectDraw7, 0);
    if (ddVal != DD_OK) return false;

	// Borderless fullscreen (windowed): no exclusive cooperative level and no display-mode switch.
	m_bFullMode = false;
	...
	// Resize the window to cover the entire monitor (borderless)
	SetWindowPos(hWnd, HWND_TOP, mi.rcMonitor.left, mi.rcMonitor.top, monitorWidth, monitorHeight, SWP_SHOWWINDOW);
...
void DXC_ddraw::ChangeDisplayMode(HWND hWnd)
{
	...
	// Borderless fullscreen (windowed): keep DDSCL_NORMAL and resize to monitor.
	m_bFullMode = false;
